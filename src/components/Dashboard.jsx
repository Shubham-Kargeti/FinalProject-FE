...
//     if (!user) return;
//     try {
//       const url = user.role === "admin" ? `${API_BASE}/admin/claims` : `${API_BASE}/claims`;
//       const res = await fetch(url, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       if (!res.ok) throw new Error("Failed to fetch claims");
...
//     }
//     try {
//       const url = user.role === "admin" ? `${API_BASE}/admin/claims/${claimId}` : `${API_BASE}/claims/${claimId}`;
//       const res = await fetch(url, {
//         headers: { Authorization: `Bearer ${token}` },
//       });
//       if (!res.ok) throw new Error(await res.text());
...
//     if (!window.confirm("Are you sure you want to delete this claim?")) return;
//     try {
//       const url = user.role === "admin" ? `${API_BASE}/admin/claims/${id}` : `${API_BASE}/claims/${id}`;
//       const res = await fetch(url, {
//         method: "DELETE",
//         headers: { Authorization: `Bearer ${token}` },
//       });
...
//   return (
//     <div className="dashboard-container">
//       <h1 className="dashboard-title">Welcome, {user.username} ({user.role})</h1>
...
//       <div className="card">
//         <h2>{user.role === "admin" ? "Manage Claims" : "Get Claim by ID"}</h2>
//         <div className="form-grid">
//           <input
//             type="number"
//             placeholder="Claim ID"
